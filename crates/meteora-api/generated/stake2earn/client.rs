...
impl HttpClient {
    ...
    pub fn with_config(enable_tracing: bool) -> Self {
        ...
        Self {
            base_url: "https://stake-for-fee-api.meteora.ag".to_string(),
            api_key: None,
            http_client,
...
#[doc = concat!("Additive request builder for `", "filter_vaults", "`.")]
#[must_use]
pub struct FilterVaultsBuilder<'a> {
    client: &'a HttpClient,
    pool_address: Option<Vec<String>>,
}
impl<'a> FilterVaultsBuilder<'a> {
    ...
    pub async fn send(self) -> Result<VaultsResponse, ApiOpError<serde_json::Value>> {
        self.client.filter_vaults(self.pool_address).await
    }
}
impl HttpClient {
    /// `GET /vault/filter`
    pub async fn filter_vaults(
        &self,
        pool_address: Option<Vec<String>>,
    ) -> Result<VaultsResponse, ApiOpError<serde_json::Value>> {
        let request_url = format!("{}{}", self.base_url, "/vault/filter");
    ...
    pub async fn get_all_analytics(
        &self,
    ) -> Result<AggregateAnalyticInfo, ApiOpError<serde_json::Value>> {
        let request_url = format!("{}{}", self.base_url, "/analytics/all");
        let mut req = self.http_client.get(request_url);
        if let Some(api_key) = &self.api_key {
            req = req.bearer_auth(api_key);
        }
    ...
    /// `GET /vault/all`
    pub async fn get_all_vaults(&self) -> Result<VaultsResponse, ApiOpError<serde_json::Value>> {
        let request_url = format!("{}{}", self.base_url, "/vault/all");
        let mut req = self.http_client.get(request_url);
        if let Some(api_key) = &self.api_key {
            req = req.bearer_auth(api_key);
        }
    ...
    pub async fn get_one_vault(
        &self,
        vault_address: impl AsRef<str>,
    ) -> Result<VaultDetailWithLeaderboard, ApiOpError<serde_json::Value>> {
        let request_url = format!(
            "{}{}",
            self.base_url,
            format!(
                "/vault/{}",
                __pct_encode_path_segment(vault_address.as_ref())
            )
        );
        let mut req = self.http_client.get(request_url);
        if let Some(api_key) = &self.api_key {
    ...
    #[doc = concat!("Start an additive builder for `", "filter_vaults", "`.")]
    pub fn filter_vaults_builder(&self) -> FilterVaultsBuilder<'_> {
        FilterVaultsBuilder {
            client: self,
            pool_address: None,
        }
